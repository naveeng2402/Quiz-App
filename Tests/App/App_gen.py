...
def Scoring_gen(self, teams, _teams, source, layout):
    ...
    for i in range(len(teams)):
        ...
        eval('''sizePolicy.setHeightForWidth(self.Team_A_Tick.sizePolicy().hasHeightForWidth())''')
        ...
        eval(f'''sizePolicy.setHeightForWidth(self.Team_A_Plus.sizePolicy().hasHeightForWidth())''')
...
def Questions_gen(self, qns, rounds, _rounds, rounds_info):
    ...
    for i in range(len(_rounds)):
        ...
        for j in range(rounds_info[rounds[i]]['nos']):
            ...
            eval(f'''self.{_rounds[i]}_Q{j+1}_Question.setFrameShape(QtWidgets.QFrame.StyledPanel)''')
